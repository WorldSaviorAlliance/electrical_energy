...
$(function()
{
		...
		$('#add').unbind('click').click(function(){
			...
			addDiv.load(rootpath + '/static/jsp/customer/dlyhDetail.jsp', function(){
				$(this).EemWindow({
		            ...
		            hasBottomBtn : false,
		            afterShow : function(){
		            	g_page_dlyh_detail = new DlyhDetail(getAllData);
		            }
		        });	
			});
		});
		...
		if(datas != null && datas.length != 0)
		{
			...
			for(var i = 0; i < datas.length; i++)
			{
				...
				trs += '<tr type="data">'+
							...
							'<td>' + getIndustryType(temp.industryType) + '</td>'+
			...
			addDiv.load(rootpath + '/static/jsp/customer/dlyhDetail.jsp', function(){
				$(this).EemWindow({
		            ...
		            afterShow : function(){
		            	...
		            	g_page_dlyh_detail = new DlyhDetail(getAllData, curData);
		            }
		        });	
			});
		});
	...
	{
		$.ajax({
			url: rootpath + '/' + PATH_DLYH + '/info?id=' + id,
			type : 'DELETE', 
			dataType: 'json',
		    contentType: 'application/json',
			complete : function(XHR, TS) {
				if (TS == "success") {
					...
					if(ar.code == 0)
					{
						showDynamicMessage(STR_CONFIRM, '删除电力用户成功', MESSAGE_TYPE_INFO);
						getAllData();
					}
					else
					{
						showDynamicMessage(STR_CONFIRM, '删除电力用户失败', MESSAGE_TYPE_INFO);
					}
				}
				else
				{
