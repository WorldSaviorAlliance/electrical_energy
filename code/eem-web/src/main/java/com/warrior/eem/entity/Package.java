package com.warrior.eem.entity;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.EnumType;
import javax.persistence.Enumerated;
import javax.persistence.Table;

import com.warrior.eem.entity.constant.PackageType;

/**
 * 套餐实体
 * 
 * @author cold_blade
 * @version 1.0.0
 */
@Entity
@Table(name = "package")
public class Package extends AbstractEntity {
	private static final long serialVersionUID = -7297933367668416632L;

	@Column(name = "name")
	private String name;

	@Column(name = "type")
	@Enumerated(EnumType.ORDINAL)
	private PackageType type;

	@Column(name = "expect_price")
	private int extPrice;

	@Column(name = "min_electricity")
	private double minElecticity;

	@Column(name = "desc")
	private String desc;

	public String getName() {
		return name;
	}

	public void setName(String name) {
		this.name = name;
	}

	public PackageType getType() {
		return type;
	}

	public void setType(PackageType type) {
		this.type = type;
	}

	public int getExtPrice() {
		return extPrice;
	}

	public void setExtPrice(int extPrice) {
		this.extPrice = extPrice;
	}

	public double getMinElecticity() {
		return minElecticity;
	}

	public void setMinElecticity(double minElecticity) {
		this.minElecticity = minElecticity;
	}

	public String getDesc() {
		return desc;
	}

	public void setDesc(String desc) {
		this.desc = desc;
	}
}
